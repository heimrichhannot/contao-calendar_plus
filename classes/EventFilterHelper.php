...
class EventFilterHelper extends \Frontend
{
    ...
    public static function getHostSelectOptions(\DataContainer $dc)
    {
        ...
        if ($objMemberHosts !== null)
        {
            ...
            {
                $arrTitle = [$objMemberHosts->academicTitle, $objMemberHosts->firstname, $objMemberHosts->lastname];
    ...
    public static function getDocentSelectOptions(\DataContainer $dc)
    {
        ...
        if ($objMemberDocents !== null)
        {
            ...
            {
                $arrTitle = [$objMemberDocents->academicTitle, $objMemberDocents->firstname, $objMemberDocents->lastname];
    ...
    public static function getCitySelectOptions(\DataContainer $dc)
    {
        ...
        $arrOptions['column'][] = "($t.endTime>=$time OR ($t.recurring='1' AND ($t.recurrences=0 OR $t.repeatEnd>=$time))) AND $t.pid IN(" . implode(
                ',',
                array_map('intval', $dc->objModule->cal_calendar)
            ) . ") AND ($t.start='' OR $t.start<='$time') AND ($t.stop='' OR $t.stop>'" . ($time + 60) . "') AND $t.published='1'";
    ...
    public static function getDatabaseResult($arrOptions = [])
    {
        $t = static::$strTable;
