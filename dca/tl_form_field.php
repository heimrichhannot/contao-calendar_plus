...
if (is_array($dc['palettes']))
{
    array_insert(
        $dc['palettes'],
        count($dc['palettes']),
        ['subEventList' => '{type_legend},type,name,label;{fconfig_legend},event,widgetType,mandatory;{options_legend},options;{expert_legend:hide},accesskey,class;{submit_legend},addSubmit']
    );
}
...
$dc['config']['onload_callback'][]   = ['tl_extended_events_form_field', 'saveOldState'];
$dc['config']['onsubmit_callback'][] = ['tl_extended_events_form_field', 'createOptions'];
...
$dc['fields']['event'] = [
    ...
    'eval'             => ['tl_class' => 'long', 'submitOnChange' => true, 'chosen' => true, 'mandatory' => true],
    'wizard'           => [
...
$dc['fields']['widgetType'] = [
    ...
    'options'   => ['radio' => 'Radio-Button-Menü', 'checkbox' => 'Checkbox-Menü'],
...
class tl_extended_events_form_field extends Backend
{
    ...
    public function editEvent(DataContainer $dc)
    {
        return ($dc->value < 1)
            ? ''
            : ' <a href="contao/main.php?do=calendar&amp;table=tl_calendar_events&amp;id=' . $dc->value . '" title="' . sprintf(
                specialchars($GLOBALS['TL_LANG']['tl_content']['editalias'][1]),
                $dc->value
            ) . '" style="padding-left:3px">' . $this->generateImage('alias.gif', $GLOBALS['TL_LANG']['tl_content']['editalias'][0], 'style="vertical-align:top"') . '</a>';
    }
