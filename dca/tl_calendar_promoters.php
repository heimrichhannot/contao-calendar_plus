...
class tl_calendar_promoters extends Backend
{
	...
	public function checkPermission()
	{
		...
		switch (Input::get('act'))
		{
			...
			case 'delete':
			case 'toggle':
				$objCalendar = $this->Database->prepare("SELECT pid FROM tl_calendar_promoters WHERE id=?")
					->limit(1)
					->execute($id);
			...
			case 'copyAll':
				...
				$objCalendar = $this->Database->prepare("SELECT id FROM tl_calendar_promoters WHERE pid=?")
					->execute($id);
