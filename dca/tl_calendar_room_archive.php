...
$GLOBALS['TL_DCA']['tl_calendar_room_archive'] = [
    'config'      => [
        'dataContainer'     => 'Table',
        'ptable'            => 'tl_calendar',
        'ctable'            => ['tl_calendar_room'],
        'enableVersioning'  => true,
        'onload_callback'   => [
            ['tl_calendar_room_archive', 'checkPermission'],
        ],
        'onsubmit_callback' => [
    ...
    'list'        => [
        'sorting'           => [
            ...
            'child_record_callback' => ['tl_calendar_room_archive', 'listRoomArchives'],
        ...
        'operations'        => [
            ...
            'toggle'     => [
                ...
                'button_callback' => ['tl_calendar_room_archive', 'toggleIcon'],
            ],
            'show'       => [
...
class tl_calendar_room_archive extends \Backend
{
    ...
    public function toggleVisibility($intId, $blnVisible, DataContainer $dc = null)
    {
        ...
        // Update the database
        $this->Database->prepare("UPDATE tl_calendar_room_archive SET tstamp=" . time() . ", published='" . ($blnVisible ? 1 : '') . "' WHERE id=?")->execute($intId);

        $objVersions->create();
        $this->log(
            'A new version of record "tl_calendar_room_archive.id=' . $intId . '" has been created' . $this->getParentEntries('tl_calendar_room_archive', $intId),
            __METHOD__,
            TL_GENERAL
        );
    ...
    public function checkPermission()
    {
        ...
        switch (Input::get('act'))
        {
            ...
            case 'copy':
                ...
                {
                    $this->log(
                        'Not enough permissions to ' . Input::get('act') . ' calendar room ID "' . $id . '" to calendar ID "' . Input::get('pid') . '"',
                        __METHOD__,
                        TL_ERROR
                    );
                    $this->redirect('contao/main.php?act=error');
            ...
            case 'delete':
            case 'toggle':
                $objRoomArchive = $this->Database->prepare("SELECT pid FROM tl_calendar_room_archive WHERE id=?")->limit(1)->execute($id);
                ...
                {
                    $this->log(
                        'Not enough permissions to ' . Input::get('act') . ' room-achives ID "' . $id . '" of calendar ID "' . $objRoomArchive->pid . '"',
                        __METHOD__,
                        TL_ERROR
                    );
                    $this->redirect('contao/main.php?act=error');
            ...
            case 'copyAll':
                ...
                $objRoomArchive = $this->Database->prepare("SELECT id FROM tl_calendar_room_archive WHERE pid=?")->execute($id);
